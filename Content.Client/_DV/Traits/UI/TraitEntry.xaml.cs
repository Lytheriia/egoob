...
using Content.Shared._Erida.Traits.Conditions;
...
public sealed partial class TraitEntry : PanelContainer
{
    ...
    /// </summary>
    public void UpdateConditionsMet(ProtoId<JobPrototype>? jobId, ProtoId<SpeciesPrototype>? speciesId, IReadOnlySet<ProtoId<AntagPrototype>>? antagPreferences, Gender? gender)
    {
        ...
        foreach (var condition in _trait.Conditions)
        {
            var result = condition switch
            {
                ...
                IsGenderCondition isGenderCondition => CheckGenderCondition(isGenderCondition, gender),
                AnyOfCondition anyOfCond => CheckAnyOfCondition(anyOfCond, jobId, speciesId, antagPreferences, gender),
                _ => true,
            };
    ...
    private bool CheckGenderCondition(IsGenderCondition condition, Gender? gender)
    {
        if (gender == null)
            return false;

        return condition.Genders.Contains(gender.Value);
    }

    private bool CheckAnyOfCondition(AnyOfCondition condition, ProtoId<JobPrototype>? jobId, ProtoId<SpeciesPrototype>? speciesId, IReadOnlySet<ProtoId<AntagPrototype>>? antagPreferences, Gender? gender)
    {
        ...
        foreach (var childCondition in condition.Conditions)
        {
            var result = childCondition switch
            {
                ...
                AnyOfCondition nestedAnyOf => CheckAnyOfCondition(nestedAnyOf, jobId, speciesId, antagPreferences, gender), // Recursive!
                IsGenderCondition isGenderCondition => CheckGenderCondition(isGenderCondition, gender),
                IsAntagEligibleCondition antagEligibleCond => CheckAntagEligibleCondition(antagEligibleCond, antagPreferences),
                _ => true,
            };
